...
`.trim();
    ...
    if (!createRes.ok) {
      console.error("[imagen-hybrid] translate create error:", prediction);
      return originalDescription;
    }
    ...
    if (result.status !== "succeeded" || !result.output) {
      console.error("[imagen-hybrid] translate prediction failed:", result);
      return originalDescription;
    }
    ...
    console.error("[imagen-hybrid] translate error:", err);
    return originalDescription;
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (type === 'custom' && prompt) {
      ...
      imagePrompt = `Create beautiful AI artwork inspired by: ${memoryTitle || 'a memory'}. ${
        memoryDescription || ''
      } Style: ${style}. Emotion: ${emotion || 'peaceful'}.`;
    } else {
    ...
    // Use Replicate non-streaming HTTP API for image generation (Imagen 4 fast)
    ...
    if (!createRes.ok) {
      console.error('Replicate create prediction error (hybrid):', prediction);
      return NextResponse.json(
        { success: false, error: prediction?.error || 'Failed to create image prediction' },
        { status: 500 }
      );
    }
    ...
    if (result.status === 'failed' || result.status === 'canceled') {
      ...
      console.error('Replicate prediction failed (hybrid):', result);
      return NextResponse.json(
    ...
    if (result.status !== 'succeeded' || result.output == null) {
      console.error('Replicate prediction did not succeed (hybrid):', result);
      return NextResponse.json(
    ...
    if (!imageUrl) {
      console.error('Unexpected Replicate prediction output format (hybrid):', result.output);
      return NextResponse.json(
        { success: false, error: 'No image URL returned from Replicate' },
        { status: 500 }
      );
...
export async function GET() {
  return NextResponse.json({
    ...
    styles: ['realistic', 'artistic', 'dreamy', 'minimalist', 'watercolor', 'sketch'],
